...
function local_sandbox_cron() {
	...
	if (!$lastcron = $DB->get_field('config_plugins', 'value', array('plugin' => 'local_sandbox', 'name' => 'lastcron'))) {
		...
		// Inform admin
		inform_admin(get_string('errorlastcronerror', 'local_sandbox'), LEVEL_ERROR);
	...
	if (strpos($config->cronrunday, "1") === false) {
		...
		// Inform admin
		inform_admin(get_string('noticedaysnotconfigured', 'local_sandbox'), LEVEL_NOTICE);
	...
	if ($config->coursebackupsdirectory != '') {
		...
		if (is_dir($config->coursebackupsdirectory)) {
			...
			if ($handle = @opendir($config->coursebackupsdirectory)) {
				while (false !== ($file = readdir($handle))) {
					if (substr($file, -4) == '.mbz' && $file != '.' && $file != '..') {
						...
						if ($oldcourse = $DB->get_record('course', array('shortname' => $shortname))) {
							...
							// Inform admin
							inform_admin(get_string('skippingnocourse', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if (!delete_course($oldcourseid, false)) {
							...
							// Inform admin
							inform_admin(get_string('skippingdeletionfailed', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if (!$zippacker->extract_to_pathname($config->coursebackupsdirectory.'/'.$file, $CFG->dataroot.'/temp/backup/'.$shortname)) {
							...
							// Inform admin
							inform_admin(get_string('skippingunzipfailed', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if (!$newcourseid = restore_dbops::create_new_course($shortname, $shortname, $categoryid)) {
							...
							// Inform admin
							inform_admin(get_string('skippingcreatefailed', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if ($controller = new restore_controller($shortname, $newcourseid, backup::INTERACTIVE_NO, backup::MODE_SAMESITE, $restoreuser, backup::TARGET_NEW_COURSE)) {
							...
							// Inform admin
							inform_admin(get_string('skippingrestorefailed', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if ($config->adjustcoursestartdate == true) {
							if (!$DB->update_record('course', (object)array('id' => $newcourseid, 'startdate' => $now))) {
								...
								// Inform admin
								inform_admin(get_string('skippingadjuststartdatefailed', 'local_sandbox', $shortname), LEVEL_WARNING);
						...
						if ($DB->update_record('course', (object)array('id' => $newcourseid, 'shortname' => $shortname, 'fullname' => $fullname))) {
							...
							// Inform admin
							inform_admin(get_string('successrestored', 'local_sandbox', $shortname), LEVEL_NOTICE);
							...
							// Inform admin
							inform_admin(get_string('skippingdbupdatefailed', 'local_sandbox', $shortname), LEVEL_WARNING);
				...
				// Inform admin
				inform_admin(get_string('noticerestorecount', 'local_sandbox', $count), LEVEL_NOTICE);
				...
				// Inform admin
				inform_admin(get_string('errordirectorynotreadable', 'local_sandbox', $config->coursebackupsdirectory), LEVEL_ERROR);
			...
			// Inform admin
			inform_admin(get_string('errordirectorynotexist', 'local_sandbox', $config->coursebackupsdirectory), LEVEL_ERROR);
		...
		// Inform admin
		inform_admin(get_string('noticedirectorynotconfigured', 'local_sandbox'), LEVEL_NOTICE);
...
function inform_admin($message, $level = LEVEL_NOTICE) {
	// Get recipients
